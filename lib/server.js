  ...
  app.post('/api/config', async (req, res) => {
    try {
      ...
      if (preset && presets[preset]) {
        ...
        let autoPreset = null
        if (userLower.includes('@qq.com') || userLower.includes('@foxmail.com')) {
          autoPreset = 'qq'
        } else if (userLower.includes('@163.com') || userLower.includes('@126.com')) {
          autoPreset = '163'
        } else if (userLower.includes('@gmail.com')) {
          autoPreset = 'gmail'
        } else if (userLower.includes('@outlook.com') || userLower.includes('@hotmail.com')) {
          autoPreset = 'outlook'
        }
        
        if (autoPreset && presets[autoPreset]) {
          config.smtp = presets[autoPreset].smtp
          config.imap = presets[autoPreset].imap
        }
      }
